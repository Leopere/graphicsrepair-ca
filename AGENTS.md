...
- Use the dedicated Notomo property ID `graphicsrepair.ca`; never reuse motherboardrepair.ca's site ID `2`. Keep full session replay enabled for this property, including literal repair-form values. Alias hosts `www.graphicsrepair.ca`, `fixgpu.ca` and `gpufix.ca` belong on that property. `graphicsrepair.com` stays a sibling property.
